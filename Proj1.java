  ...
  /*
	...
	public static class Map1 extends
		...
		public void map(WritableComparable docID, Text docContents,
			...
			// find indices of target word:
			ArrayList<Integer> indexList = new ArrayList<Integer>();
			for (int i = 0; i < list.size(); i++) {
				if (targetGram.equals(list.get(i))) {
					indexList.add(i);
				}
			}
			
			//if there are no target words:
			if (indexList.size() == 0) {
				for (int j = 0; j < list.size(); j++) {
					context.write(new Text(list.get(j)), new DoublePair(1.0, func.f(Double.POSITIVE_INFINITY)));
				}
				return;
			}
 
			
			// compare other indices with target word indices:
			int distance = 0;
			int indexFirst = indexList.get(0);
			int indexLast = indexList.get(indexList.size() - 1);
			int indexCount = 0;
			...
			for (int k = 0; k < list.size(); k++) {
				...
				// case 1: hasn't reached first target word yet:
				if (k < indexFirst) {
					distance = indexFirst - k;
				}
 
				// case 2: between two target words:
 
				if (k == indexList.get(indexCount) && k != indexLast) {
					indexCount++;
					continue;
				}
 
				if (k > indexFirst && k < indexLast) {
					distance = Math.min(k - indexList.get(indexCount),
							indexList.get(indexCount + 1) - k);
					
				}
 
				// case 3: reached last target word
				...
				if (k > indexLast) {
					distance = k - indexLast;
				}
 
				context.write(new Text(list.get(k)),new DoublePair(1.0, func.f(distance)));
				System.out.println("word: " + list.get(k) + " func: " + func.f(distance));
			}
 
 
			// end goal: emit (word, duoblePair(1.0, func.f(distance))
		}
	...
	public static class Combine1 extends Reducer<Text, DoublePair, Text, DoublePair> {
		...
		public void reduce(Text key, Iterable<DoublePair> values, Context context)
			...
			for (DoublePair value : values) {
				totalOcc = totalOcc + value.getDouble1();
				fdw = fdw + value.getDouble2();
			}
 
			context.write(key, new DoublePair(totalOcc, fdw));
	...
	public static class Reduce1 extends
		...
		public void reduce(Text key, Iterable<DoublePair> values, Context context)
			...
			//for all the same key values, implement co-occurrence
			double totalOcc = 0;
			double fdw = 0;
			for (DoublePair value : values ) {
				totalOcc = totalOcc + value.getDouble1();
			...
			double rate; 
			if (fdw > 0) {
				rate = fdw * Math.pow(Math.log(fdw), 3) / totalOcc;
			} else {
				rate = 0.0;
			}
 
			DoubleWritable doubleWriter = new DoubleWritable(-1*rate);
			
			context.write(doubleWriter, key);
			
			// goal: implement co-occurrence equation
	...
	public static class Reduce2 extends
		...
		public void reduce(DoubleWritable key, Iterable<Text> values,
			...
			// YOUR CODE HERE
			// goal: only input 100 and order them correctly
